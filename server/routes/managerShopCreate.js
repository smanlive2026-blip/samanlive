...
const Product = require('../models/Product'); // ✅ NEW: Product model add kiya
const CategoryMaster = require('../models/CategoryMaster'); // ✅ NEW: CategoryMaster model add kiya
...
router.post('/create-shop-v2', authManager, async (req, res) => {
    try {
        ...
        const newShop = new Shop({
            ...
            serviceType: shopType, // ✅ yahi category hai
        ...
        await newShop.save();
        console.log('✅ New shop created:', shopName, '| ID:', newShop._id);

        // ========== 🔥 NEW CODE START: AUTO PRODUCT COPY ==========
        try {
            const template = await CategoryMaster.findOne({ category: shopType });
            if(template && template.products.length > 0){
                ...
                console.log(`✅ ${productsToInsert.length} template products copied to ${shopName}`);
            }
        } catch(err){
            console.error('⚠️ Template product copy failed:', err.message);
        }
        // ========== 🔥 NEW CODE END ==========

        res.json({
            success: true,
            message: 'Shop created successfully with template products',
            shop: newShop,
