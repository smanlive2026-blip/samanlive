...
// CHANGED: loadShopData ko async kiya aur API se load kiya
async function loadShopData() {
    try {
        document.getElementById('loader').innerText = 'Loading...';

        // CHANGED: API SE DATA UTHAO, LOCALSTORAGE HATAO
        const res = await fetch(`/api/shops/${shopId}`);
        const result = await res.json();

        if(!result.success) throw new Error('Shop not found');
        const shop = result.shop;

        // 1. Shop Name + Photo + Settings
        document.getElementById('shopName').innerText = shop.shopName || 'Fresh Fruits';
        if(shop.ownerPhotoUrl) document.getElementById('ownerPhoto').src = shop.ownerPhotoUrl; // CHANGED: DB se photo

        document.getElementById('announcementText').value = shop.announcement || ''; // CHANGED
        document.getElementById('openTime').value = shop.shopSettings?.openTime || '08:00'; // CHANGED
        document.getElementById('closeTime').value = shop.shopSettings?.closeTime || '22:00'; // CHANGED

        // 2. Load Fruits: Ab DB se
        allShopFruits = shop.items && shop.items.length > 0? shop.items : [...window.FRUIT_PRODUCTS_DATA]; // CHANGED

        // 3. Stats Calculate - ye local hi rahega
        const totalStock = allShopFruits.reduce((sum, f) => sum + (f.stock || 0), 0);
        const totalCustomers = Math.floor(Math.random()*200) + 50; // abhi local hi hai
        const revenue = allShopFruits.reduce((sum, f) => sum + (f.price * 2), 0);
        const todaySales = Math.floor(Math.random()*20) + 5;
        ...
        // 4. OPEN/CLOSE
        document.getElementById('shopToggle').checked = shop.isOpen; // CHANGED
        updateStatusBadge(shop.isOpen); // CHANGED
        ...
        console.error(err);
        document.getElementById('loader').innerText = 'Failed to load data from server';
    }
}

// CHANGED: NAYA FUNCTION BANAYA BADGE KE LIYE
...
function loadFruits(fruits) { /*... ye same rahega... */
    ...
    tbody.innerHTML = fruits.map(fruit => {
        ...
        return `<tr><td><div style="display:flex; align-items:center; gap:10px;"><img src="${fruit.image || 'https://via.placeholder.com/40?text=F'}" style="width:40px; height:40px; border-radius:8px; object-fit:cover;"><strong>${fruit.name}</strong></div></td><td>₹<strong>${fruit.price}</strong> <span style="font-size:12px; color:#64748b;">/${fruit.unit}</span></td><td class="${stockClass}"><strong>${stock}</strong> ${fruit.unit}</td><td>${status}</td><td style="display:flex; gap:5px; flex-wrap:wrap;"><button onclick="editFruit('${fruit._id || fruit.id}')" style="background:#22c55e; color:white; border:none; padding:6px 10px; border-radius:8px; cursor:pointer;"><i class="fa fa-edit"></i></button><button onclick="deleteFruit('${fruit._id || fruit.id}')" style="background:#ef4444; color:white; border:none; padding:6px 10px; border-radius:8px; cursor:pointer;"><i class="fa fa-trash"></i></button></td></tr>`;
    }).join('');
}

// CHANGED: DELETE AB DB ME BHI KAREGA
async function deleteFruit(fruitId) {
    if(confirm('Are you sure you want to delete this fruit?')) {
        ...
        await fetch(`/api/shops/${shopId}`, {
            ...
            body: JSON.stringify({ items: allShopFruits })
        });
        loadShopData();
    }
}
...
// CHANGED: TOGGLE AB DB ME SAVE KAREGA
document.getElementById('shopToggle').onchange = async (e) => {
    updateStatusBadge(e.target.checked);
    await fetch(`/api/shops/${shopId}`, {
        method: 'PUT',
        headers: {'Content-Type': 'application/json'},
        body: JSON.stringify({ isOpen: e.target.checked })
    });
}

// CHANGED: TIMINGS AB DB ME SAVE
async function saveTimings() {
    ...
    await fetch(`/api/shops/${shopId}`, {
        method: 'PUT',
        headers: {'Content-Type': 'application/json'},
        body: JSON.stringify({ shopSettings: { openTime: open, closeTime: close } })
    });
    alert('Timings Saved!');
}

// CHANGED: ANNOUNCEMENT AB DB ME SAVE
...
// CHANGED: MANUAL UPLOAD ME BHI DB SAVE ADD
